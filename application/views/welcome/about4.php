<!DOCTYPE html>
<html>

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1,  user-scalable=no">
    <meta name="format-detection" content="telephone=no">
    <title></title>
    <link rel="stylesheet" href="<?=SITE_URL;?>/static/css/bootstrap.min.css">
    <link rel="stylesheet" href="<?=SITE_URL;?>/static/css/init.css">
    <link rel="stylesheet" href="<?=SITE_URL;?>/static/css/main.css">
    <script type="text/javascript" src="<?=SITE_URL;?>/static/js/jquery-1.7.2.js"></script>
    <script type="text/javascript" src="<?=SITE_URL;?>/static/js/bootstrap.min.js"></script>
</head>

<body style="background: #ebeaea;">
    <div class="head ny_head">
            <?php echo template('public','head')?>

        <!--head start-->
        <!--#include file="inc/head.htm" -->
        <!--head end-->
        <div class="head_b">
            关于雷瓦
        </div>
    </div>
    <div class="about_tl">
        <p class="about_tl_t">
            ABOUT RIWA
        </p>
        <div class="about_menu">
            <a href="about">品牌故事</a>
            <a href="about4" class="ahover">品牌印记</a>
            <a href="about2">品牌理念</a>
            <a href="about3">核心实力</a>
        </div>
    </div>
    <div class="about_ban">
        <img src="<?=SITE_URL;?>/static/images/about4ban_02.jpg">
        <div class="about4_ban_text">
            品牌印记
            <p class="about4_ban_text1 eng">
                <small>RIWA</small> IMPRINT
            </p>
        </div>
    </div>
    <div class="container">
        <div class="about4_box img1">
            <div class="txt">WE HAVE BEEN MOVING</div>
            <div class="dot"><img src="<?=SITE_URL;?>/static/images/about4/about4_01_dot.png" alt=""></div>
            <div class="year">201<em>6</em></div>
            <div class="histroy">RIWA雷瓦推出全新Everchanging百变造型器RC-7506，换“型”概念，开创美发造型新时代。</div>
            <img src="<?=SITE_URL;?>/static/images/about4/about4_01.jpg" alt="">
        </div>
        <div class="about4_box img2">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_02.jpg" alt="">
            <div class="year">201<em>6</em></div>
            <div class="histroy">世界知识产权组织（WPIO）认定RIWA（雷瓦） 为国际商标，在全球60个国家范围受到保护
            </div>
        </div>
        <div class="about4_box img3">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_03.jpg" alt="">
            <div class="year">201<em>5</em></div>
            <div class="histroy">美容器R3获得<br> 中国家电艾普兰奖
                <br> 最受大众欢迎产品奖。
            </div>
        </div>
        <div class="about4_box img4">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_04.jpg" alt="">
            <div class="year">201<em>4</em></div>
            <div class="histroy">RIWA雷瓦荣获商派电商大会金商奖。</div>
        </div>
        <div class="about4_box img5">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_05.jpg" alt="">
            <div class="year">201<em>3</em></div>
            <div class="histroy">女士剃毛器RF-770A荣获中国设计红星奖及韩国好设计两项大奖。</div>
        </div>
        <div class="about4_box img6">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_06.jpg" alt="">
            <div class="year">201<em>2</em></div>
            <div class="histroy">RIWA雷瓦首次推出颠覆性的迷你型造型器RB-809S，使造型产品进入便携时代</div>
        </div>
        <div class="about4_box img7">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_07.jpg" alt="">
            <div class="year">201<em>2</em></div>
            <div class="histroy">成立上海雷瓦电器有限公司，正式进入中国市场。</div>
        </div>
        <div class="about4_box img8">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_08.jpg" alt="">
            <div class="year">201<em>0</em></div>
            <div class="histroy">研发的直发造型器在COSMOPROF亚太美容展上 大放异彩，获最受欢迎美发工具品牌荣誉。
            </div>
        </div>
        <div class="about4_box img9">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_09.jpg" alt="">
            <div class="year">200<em>9</em></div>
            <div class="histroy">与意大利美发时尚品牌Müster & Dikson S.p.a达成长期合作，组建国际造型器研发中心。</div>
        </div>
        <div class="about4_box img10">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_10.jpg" alt="">
            <div class="year start">2000</div>
        </div>
        <div class="about4_box img11">
            <img src="<?=SITE_URL;?>/static/images/about4/about4_11.jpg" alt="">
        </div>
    </div>


    <div class="caitiao container">
        <img src="<?=SITE_URL;?>/static/images/about1_46.png">
    </div>
            <?php echo template('public','footer')?>

    <!--footer start-->
    <!--#include file="inc/footer.htm" -->
    <!--footer end-->

</body>

</html>