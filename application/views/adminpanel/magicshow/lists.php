...
<div class='panel panel-default grid'>
    <div class='panel-heading'>
        <i class='fa fa-table'></i> 魔发秀列表
        <div class='panel-tools'>
            <div class='btn-group'>
                 <a class="btn " href="<?php echo base_url('adminpanel/magicshow/add')?>"><span class="glyphicon glyphicon-plus"></span> 添加 </a>             </div>
...
<button type="submit" name="dosubmit" value="搜索" class="btn btn-success"><i class='glyphicon glyphicon-search'></i></button>        </form>
          ...
          <form method="post" id="form_list"  action="<?php echo base_url('adminpanel/magicshow/delete_all')?>"  > 
    ...
    <?php if($data_list):?>
        <table class="table table-hover dataTable" id="checkAll">
          <thead>
            <tr>
              <th>#</th>
                            <?php $css=""; $next_url = base_url('adminpanel/magicshow?order=type&dir=desc'); ?>
              <?php if(($order=='type'&&$dir=='desc')) { ?>
              <?php $css="sorting_desc";$next_url = base_url('adminpanel/magicshow?order=type&dir=asc'); ?>
              ...
              <?php } ?><th class="sorting <?php echo $css;?>"   onclick="window.location.href='<?php echo $next_url;?>'"   nowrap="nowrap">类型</th>
                            <?php $css=""; $next_url = base_url('adminpanel/magicshow?order=title&dir=desc'); ?>
              <?php if(($order=='title'&&$dir=='desc')) { ?>
              <?php $css="sorting_desc";$next_url = base_url('adminpanel/magicshow?order=title&dir=asc'); ?>
              ...
              <?php } ?><th class="sorting <?php echo $css;?>"   onclick="window.location.href='<?php echo $next_url;?>'"   nowrap="nowrap">标题</th>
                            <?php $css=""; $next_url = base_url('adminpanel/magicshow?order=product&dir=desc'); ?>
              <?php if(($order=='product'&&$dir=='desc')) { ?>
              <?php $css="sorting_desc";$next_url = base_url('adminpanel/magicshow?order=product&dir=asc'); ?>
              <?php } elseif (($order=='product'&&$dir=='asc')) { ?>
              <?php $css="sorting_asc";?>
          ...
          <?php foreach($data_list as $k=>$v):?>
            <tr>
              <td><input type="checkbox" name="pid[]" value="<?php echo $v['magicshow_id']?>" /></td>
                             <td><?php echo $v['type']?></td>
                            <td><?php echo $v['title']?></td>
                            <td><?php echo $v['product']?></td>
                            <td><?php echo $v['createtime']?></td>
              <td>
                            	<a href="<?php echo base_url('adminpanel/magicshow/readonly/'.$v['magicshow_id'])?>"  class="btn btn-default btn-xs"><span class="glyphicon glyphicon-share-alt"></span> 查看</a>
                                            <a href="<?php echo base_url('adminpanel/magicshow/edit/'.$v['magicshow_id'])?>"  class="btn btn-default btn-xs"><span class="glyphicon glyphicon-edit"></span> 修改</a>
                                            <button type="button" class="btn btn-default btn-xs delete-btn" value="<?php echo $v['magicshow_id'];?>"><span class="glyphicon glyphicon-remove"></span> 删除</button>
...
<?php endif;?>
    ...
    require(['<?php echo SITE_URL?>scripts/common.js'], function (common) {
        require(['<?php echo SITE_URL?>scripts/adminpanel/magicshow/lists.js']);
    });
</script>
