 ...
 */
class Showgroud extends Admin_Controller {
	
    var $method_config;
    function __construct()
	{
		parent::__construct();
		$this->load->model(array('showgroud_model'));
    ...
    function index($page_no=0,$sort_id=0)
    {
        ...
        if (isset($_GET['dosubmit'])) {
			...
			if($_arr['keyword']!="") $where_arr[] = "concat(name,content) like '%{$_arr['keyword']}%'";
        	...
        	$data_list = $this->showgroud_model->listinfo($where,'*',$orderby , $page_no, $this->showgroud_model->page_size,'',$this->showgroud_model->page_size,page_list_url('adminpanel/showgroud/index',true));
        if($data_list)
        {
    	...
    	$this->view('lists',array('require_js'=>true,'data_info'=>$_arr,'order'=>$order,'dir'=>$dir,'data_list'=>$data_list,'pages'=>$this->showgroud_model->pages));
    }
    ...
    function add()
    {
    	...
    	if($this->input->is_ajax_request())
		{
			...
			$_arr['date'] = isset($_POST["date"])?trim(safe_replace($_POST["date"])):'';
			$_arr['createtime'] = date('Y-m-d H:i:s');
			
            $new_id = $this->showgroud_model->insert($_arr);
            if($new_id)
            {
        ...
        {
        	$this->view('edit',array('require_js'=>true,'is_edit'=>false,'id'=>0,'data_info'=>$this->showgroud_model->default_info()));
        }
    }
     /**
     * 删除单个数据
    ...
    {
    	$id = intval($id);
        $data_info =$this->showgroud_model->get_one(array('showgroud_id'=>$id));
        if(!$data_info)$this->showmessage('信息不存在');
        $status = $this->showgroud_model->delete(array('showgroud_id'=>$id));
        if($status)
        {
    ...
    function delete_all()
    {
        if(isset($_POST))
		{
			...
			$where = $this->showgroud_model->to_sqls($pidarr, '', 'showgroud_id');
			$status = $this->showgroud_model->delete($where);
			if($status)
			{
    ...
    function edit($id=0)
    {
    	...
    	if($this->input->is_ajax_request())
		{
            ...
            $status = $this->showgroud_model->update($_arr,array('showgroud_id'=>$id));
            if($status)
            {
...
/* End of file showgroud.php */
/* Location: ./showgroud.php */
?>