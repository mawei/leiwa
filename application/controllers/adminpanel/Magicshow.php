 ...
 */
class Magicshow extends Admin_Controller {
	
    var $method_config;
    function __construct()
	{
		parent::__construct();
		$this->load->model(array('magicshow_model'));
    ...
    function index($page_no=0,$sort_id=0)
    {
        ...
        if (isset($_GET['dosubmit'])) {
			...
			if($_arr['keyword']!="") $where_arr[] = "concat(title,product) like '%{$_arr['keyword']}%'";
        	...
        	$data_list = $this->magicshow_model->listinfo($where,'*',$orderby , $page_no, $this->magicshow_model->page_size,'',$this->magicshow_model->page_size,page_list_url('adminpanel/magicshow/index',true));
        if($data_list)
        {
    	...
    	$this->view('lists',array('require_js'=>true,'data_info'=>$_arr,'order'=>$order,'dir'=>$dir,'data_list'=>$data_list,'pages'=>$this->magicshow_model->pages));
    }
    ...
    function add()
    {
        ...
        {
        	$this->view('edit',array('require_js'=>true,'is_edit'=>false,'id'=>0,'data_info'=>$this->magicshow_model->default_info()));
        }
    }
     /**
     * 删除单个数据
    ...
    {
    	$id = intval($id);
        $data_info =$this->magicshow_model->get_one(array('magicshow_id'=>$id));
        if(!$data_info)$this->showmessage('信息不存在');
        $status = $this->magicshow_model->delete(array('magicshow_id'=>$id));
        if($status)
        {
    ...
    function delete_all()
    {
        if(isset($_POST))
		{
			...
			$where = $this->magicshow_model->to_sqls($pidarr, '', 'magicshow_id');
			$status = $this->magicshow_model->delete($where);
			if($status)
			{
...
/* End of file magicshow.php */
/* Location: ./magicshow.php */
?>