	function uploadOneFile(inputId,w,h,iscallback){
		...
		var window_url = SITE_URL+'adminpanel//productimage/upload/';
	...
	function  chooseWindowProduct_id(inputId,w,h,iscallback){
		...
		if(!h) h=screen.height-95;
		if(!iscallback)iscallback=0;
		var window_url = SITE_URL+'adminpanel/product/product_window/';
		$.extDialogFrame(window_url+inputId,{model:true,width:w,height:h,title:'请选择...',buttons:null});
	}
	function getProduct_id(v,t){
		$("#product_id").val(v);
		$("#product_id_text").val(t);
		$("#dialog" ).dialog();$("#dialog" ).dialog("close");
	}
...
$("#product_id_text").click(function(){
chooseWindowProduct_id('product_id',800,550,1,2)
})
            $('#validateform').bootstrapValidator({
				...
				$.ajax({
					type: "POST",
					url: is_edit?SITE_URL+"adminpanel/productimage/edit/"+id:SITE_URL+"adminpanel/productimage/add/",
					...
					success:function(response){
						...
						if(dataObj.status)
						{
							...
							aci.GoUrl(SITE_URL+'adminpanel/productimage/',1);
						}else
						{
